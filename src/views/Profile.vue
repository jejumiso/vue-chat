<template>
	...
	<div>
		<div>
			<h3>등록된 이미지</h3>
			<img v-for="photo in this.photos" :src="`${baseUrl}/uploads/${photo}`" :alt="photo" v-bind:key="photo" style="width:100px;height:100px" />
		</div>
		<div class="main-container">
            ...
            <img :src="profileImage" />
				...
				<div class="room-picture-notice" style="text-align:center">
					<button @click="submit">등록</button>
					<button @click="submit2">등록submit2</button>
				</div>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
		...
		submit2() {
			...
			formData.append('sj', this.$store.state.nickname); // 컨트롤러 넘길 정보 예 1
			// formData.append('area', 'area'); // 컨트롤러 넘길 정보 예 2 // 이미지
			this.$axios
				.post(this.baseUrl + 'upload/x', formData)
				.then(response => {
					console.log('response : ', JSON.stringify(response, null, 2));
				})
				.catch(error => {
					console.log('1. failed : ', error);
				});
		},
		submit() {
			var formData = new FormData();
			formData.append('nickname', this.$store.state.nickname); // 컨트롤러 넘길 정보 예 1
			// formData.append('area', 'area'); // 컨트롤러 넘길 정보 예 2 // 이미지
			if (this.files.length > 0) {
				...
				this.$axios
					.post(this.baseUrl + 'upload/test', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
					.then(response => {
						console.log('response : ', JSON.stringify(response, null, 2));
						this.photos = response.data.split('|');
			...
			// this.$axios
			// 	.post('http://localhost:8080/upload/test', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
			// 	.then(response => {
			...
			// 	});
			// this.$axios
			// 	.post('http://localhost:8080/upload/test', formData)
			// 	.then(response => {
			// 		console.log('response : ', JSON.stringify(response, null, 2));
			// 	})
			// 	.catch(error => {
